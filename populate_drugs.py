"""
💊 populate_drugs.py — Скрипт генерации тестовых данных (препаратов и движений)

Назначение:
Автоматически создаёт случайные препараты и связанные с ними движения (приход/расход).
Полезно при тестировании интерфейса, отчётов, фильтрации и прочего функционала проекта.

Что делает скрипт:
- Создаёт 120 препаратов со случайными названиями, формами, производителями и сроками годности;
- Для каждого препарата добавляет от 3 до 5 записей о движениях (поступление или назначение);
- Движения создаются с датами из последних 60 дней;
- Каждое движение указывает администратора как автора (если он есть в базе).

Как запускать:
  python populate_drugs.py  (после настройки DJANGO_SETTINGS_MODULE)

Внимание:
  Скрипт напрямую работает с базой данных. Убедитесь, что вы находитесь в нужной среде (dev/test).
"""
