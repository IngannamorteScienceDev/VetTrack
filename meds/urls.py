"""
urls.py — Маршруты (URL-конфигурация) приложения

Определяет, какие функции views обрабатывают какие адреса:
- /drugs/ — список препаратов;
- /drugs/create/ — добавление препарата;
- /drugs/<id>/movement/ — запись движения;
- /report/ — отчёты и выгрузка в Excel;
- и другие.

Используется Django path() для связывания URL с представлениями.
"""
